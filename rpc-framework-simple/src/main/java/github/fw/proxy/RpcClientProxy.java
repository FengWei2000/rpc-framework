...
import github.fw.remoting.transport.netty.client.NettyRpcClient;
...
//            rpcResponse = completableFuture.get();  // get()方法是阻塞的
            rpcResponse = completableFuture.get(5, TimeUnit.SECONDS);  // get()方法是阻塞的，设置超时时间为5秒
        }
        if (rpcRequestTransport instanceof SocketRpcClient) {
            rpcResponse = (RpcResponse<Object>) rpcRequestTransport.sendRpcRequest(rpcRequest);
        }
        // 检查rpcResponse是否为空、requestId是否相同、code是否成功
